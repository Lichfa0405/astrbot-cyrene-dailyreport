...
def get_next_year_festival_date(festival_name: str, current_festival_date: date) -> date:
    """获取下一个该节日的日期"""
...
def get_lunar_date_str(today=None) -> str:
    """获取农历日期字符串，供顶部日期卡使用，如 '八月初七'"""
    ...
    zhdata = ZhDate.from_datetime(_dt.combine(today, _dt.min.time()))
    return zhdata.chinese().split()[0][5:]